pipeline {
  ...
  stages {
    ...
    stage('Run Test') {
      steps {
        sh '''docker run --rm -e SELENIUM_HUB=selenium-hub-${BUILD_NUMBER} -e BROWSER=firefox -e MODULE=search-module.xml -v ${WORKSPACE}/search:/usr/share/tag/test-output --network jenkins-${BUILD_NUMBER} pugazvel/containertest
